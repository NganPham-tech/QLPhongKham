...
namespace QLPhongKham.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                    ...
                    lockoutOnFailure: false);                if (result.Succeeded)
                {
                    ...
                    if (user != null)
                    {
                        var roles = await _userManager.GetRolesAsync(user);                        if (roles.Contains("Admin"))
                        {
                            _logger.LogInformation("Admin user logged in, redirecting to Admin area.");
                            // Sử dụng LocalRedirect thay vì RedirectToAction để tránh vòng lặp chuyển hướng
                        ...
                        {
                            return RedirectToAction("Index", "Home", new { area = "BenhNhan" });
                        }
                    }

                    // Nếu không có role đặc biệt hoặc role không được nhận diện, điều hướng về trang chủ
